...
export const TrackInfoModal: React.FC<TrackInfoModalProps> = ({ track, open, onOpenChange, onSave }) => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="album" value={formData.album} onChange={e => setFormData({...formData, album: e.target.value})} className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="bpm" className="text-right">BPM</Label>
            <div className="col-span-3 flex gap-2">
              <Input id="bpm" value={formData.bpm} onChange={e => setFormData({...formData, bpm: e.target.value})} />
              <Button size="icon" variant="outline" onClick={handleReanalyze} disabled={isAnalyzing}>
                <RefreshCw className={isAnalyzing ? "h-4 w-4 animate-spin" : "h-4 w-4"} />
              </Button>
            </div>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="key" className="text-right">Key (Camelot)</Label>
            <Input id="key" value={formData.camelot_key} onChange={e => setFormData({...formData, camelot_key: e.target.value})} className="col-span-3" />
          </div>
        </div>
        <DialogFooter>
