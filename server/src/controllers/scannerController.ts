...
export const getBootstrap = (_req: Request, res: Response) => {
    const folderCount = (db.prepare('SELECT COUNT(*) as c FROM watched_folders').get() as { c: number }).c;
    const row = db
...
export const postOnboardingHome = async (_req: Request, res: Response) => {
    ...
    db.prepare('INSERT OR IGNORE INTO watched_folders (path, added_at, auto_discovered) VALUES (?, ?, 1)').run(
        home,
        Date.now(),
    );
    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run(
...
export const postOnboardingChooseFolder = async (req: Request, res: Response) => {
    ...
    db.prepare('INSERT OR IGNORE INTO watched_folders (path, added_at, auto_discovered) VALUES (?, ?, 0)').run(
        trimmed,
        Date.now(),
    );
    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run(
