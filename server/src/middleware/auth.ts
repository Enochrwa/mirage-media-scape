...
export const JWT_SECRET =
  ...
  (() => {
    const existing = db.prepare('SELECT value FROM settings WHERE key = ?').get('jwt_secret') as
      | { value: string }
      | undefined;
...
export const authMiddleware: RequestHandler = (req: AuthRequest, res, next) => {
  ...
  if (isLocal) {
    let user = db.prepare("SELECT id, username, role FROM users WHERE username = 'local'").get() as
