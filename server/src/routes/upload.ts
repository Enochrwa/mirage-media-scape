...
router.post('/', authMiddleware, upload.single('file'), async (req: Request, res) => {
  ...
  try {
    ...
    const userStorage = db
      .prepare('SELECT SUM(file_size) as total FROM tracks WHERE owner_id = ?')
    ...
    setImmediate(() => {
      try {
        native.generateWaveform(filePath);
      } catch (_e) {
        console.error(_e);
      }
    });
...
router.delete('/:trackId', authMiddleware, (req: AuthRequest, res) => {
  const track = db.prepare('SELECT * FROM tracks WHERE id = ?').get(req.params.trackId) as
    | { owner_id: string; upload_path: string | null }
    | undefined;
