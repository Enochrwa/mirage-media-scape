...
router.post('/tracks/:trackId/like', authMiddleware, (req: AuthRequest, res) => {
  ...
  const existing = db.prepare('SELECT 1 FROM track_likes WHERE user_id = ? AND track_id = ?').get(userId, trackId);
  if (existing) {
    db.prepare('DELETE FROM track_likes WHERE user_id = ? AND track_id = ?').run(userId, trackId);
  } else {
    db.prepare('INSERT INTO track_likes (user_id, track_id, liked_at) VALUES (?, ?, ?)')
...
router.post('/tracks/:trackId/comments', authMiddleware, (req: AuthRequest, res) => {
  ...
  db.prepare('INSERT INTO track_comments (id, user_id, track_id, body, created_at, parent_id) VALUES (?, ?, ?, ?, ?, ?)')
    .run(id, req.user!.id, trackId, body, Date.now(), parentId || null);

  const comment = db.prepare('SELECT tc.*, u.username FROM track_comments tc JOIN users u ON tc.user_id = u.id WHERE tc.id = ?').get(id);
  res.json({ comment });
});
...
router.post('/users/:userId/follow', authMiddleware, (req: AuthRequest, res) => {
  ...
  const existing = db.prepare('SELECT 1 FROM follows WHERE follower_id = ? AND followed_id = ?').get(followerId, userId);
  if (existing) {
    db.prepare('DELETE FROM follows WHERE follower_id = ? AND followed_id = ?').run(followerId, userId);
  } else {
    db.prepare('INSERT INTO follows (follower_id, followed_id, followed_at) VALUES (?, ?, ?)').run(followerId, userId, Date.now());
  }
