...
export class ScannerService {
    ...
    static async getLibraryStats() {
        ...
        const totalDuration = db.prepare('SELECT SUM(duration) as duration FROM tracks').get() as { duration: number };
        const artists = db.prepare('SELECT COUNT(DISTINCT artist) as count FROM tracks').get() as { count: number };
