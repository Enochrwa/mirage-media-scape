...
parentPort?.on(
  'message',
  (msg: { type: string; files: { path: string; mtime: number; size: number }[] }) => {
    if (msg.type === 'PROCESS_BATCH') {
      ...
      for (const file of files) {
        try {
          ...
          const existing = db
            .prepare(
              'SELECT last_modified, id, file_size, fingerprint FROM tracks WHERE file_path = ?',
            )
            .get(filePath) as
            | { last_modified: number; id: string; file_size: number; fingerprint: string | null }
            | undefined;
