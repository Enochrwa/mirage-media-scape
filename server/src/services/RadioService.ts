...
export class RadioService {
  ...
  async search(query: string, limit: number = 20) {
    const res = await fetch(`${this.apiBase}/stations/search?name=${encodeURIComponent(query)}&limit=${limit}`);
  ...
  async getByTag(tag: string, limit: number = 50) {
    const res = await fetch(`${this.apiBase}/stations/bytag/${encodeURIComponent(tag)}?limit=${limit}`);
    const stations = await res.json() as any[];
    this.cacheStations(stations);
    return stations;
  }
  ...
  async toggleFavorite(stationuuid: string) {
    const existing = this.db.prepare('SELECT 1 FROM radio_favorites WHERE stationuuid = ?').get(stationuuid);
    if (existing) {
      this.db.prepare('DELETE FROM radio_favorites WHERE stationuuid = ?').run(stationuuid);
    } else {
      const s = this.db.prepare('SELECT * FROM radio_stations WHERE stationuuid = ?').get(stationuuid) as any;
      if (s) {
        this.db.prepare(`
          INSERT INTO radio_favorites (stationuuid, name, url, favicon, added_at)
          VALUES (?, ?, ?, ?, ?)
        `).run(s.stationuuid, s.name, s.url, s.favicon, Math.floor(Date.now() / 1000));
      }
    }
  }
