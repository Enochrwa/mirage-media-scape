...
export const MOOD_PROFILES: Record<string, MoodProfile> = {
  focus: {
    bpmRange: [60, 100],
    energyMax: 0.5,
    genres: ['classical', 'ambient', 'jazz', 'study', 'instrumental'],
  },
  workout: {
    bpmRange: [128, 180],
    energyMin: 0.7,
    genres: ['electronic', 'hip-hop', 'rock', 'edm', 'pop'],
  },
  worship: {
    bpmRange: [60, 90],
    energyMax: 0.6,
    genres: ['gospel', 'christian', 'worship', 'spiritual', 'soul'],
  },
  sleep: {
    bpmRange: [40, 70],
    energyMax: 0.3,
    genres: ['ambient', 'classical', 'acoustic', 'nature', 'drone'],
  },
  party: {
    bpmRange: [120, 160],
    energyMin: 0.75,
    genres: ['pop', 'dance', 'electronic', 'afrobeats', 'reggaeton'],
  },
  chill: {
    bpmRange: [70, 110],
    energyMax: 0.6,
    genres: ['lo-fi', 'jazz', 'indie', 'soul', 'r&b'],
  },
};
...
export class RadioService {
  ...
  async search(query: string): Promise<RadioBrowserStation[]> {
    try {
      const res = await fetch(
        `${RadioService.API_URL}/stations/search?name=${encodeURIComponent(query)}&limit=20&hidebroken=true`,
      );
      if (res.ok) {
  ...
  async getStationsByTag(tag: string): Promise<RadioBrowserStation[]> {
    try {
      const res = await fetch(
        `${RadioService.API_URL}/stations/bytag/${encodeURIComponent(tag)}?limit=50&hidebroken=true`,
      );
      if (res.ok) {
        const data = (await res.json()) as RadioBrowserStation[];
        this.cacheStations(data);
  ...
  private cacheStations(stations: RadioBrowserStation[]) {
