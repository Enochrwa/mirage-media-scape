...
export class StatsService {
    static getGlobalStats() {
        ...
        const totalPlays = db.prepare('SELECT COUNT(*) as count FROM play_events WHERE completed = 1 OR position > 30').get() as { count: number };
        const totalTime = db.prepare('SELECT SUM(position) as time FROM play_events').get() as { time: number };
