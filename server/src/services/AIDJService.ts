...
import { type TrackMetadata } from '../../zovyra-native';

export class AIDJService {
  public generateIntro(prevTrack: TrackMetadata, nextTrack: TrackMetadata) {
    const bestScript = DJ_SCRIPTS[Math.floor(Math.random() * DJ_SCRIPTS.length)];

    // Placeholder replacement
    const intro = bestScript
      ...
      .replace(
        /{bpm}/g,
        Math.round((nextTrack as unknown as { bpm?: number })?.bpm || 120).toString(),
      )
      .replace(
        /{prevBpm}/g,
        Math.round((prevTrack as unknown as { bpm?: number })?.bpm || 120).toString(),
      )
      .replace(/{year}/g, (nextTrack?.year || 'recent times').toString())
      .replace(
        /{key}/g,
        (nextTrack as unknown as { camelotKey?: string })?.camelotKey ||
          (nextTrack as unknown as { key?: string })?.key ||
          'a new key',
      )
      .replace(
        /{prevKey}/g,
        (prevTrack as unknown as { camelotKey?: string })?.camelotKey ||
          (prevTrack as unknown as { key?: string })?.key ||
          'the previous key',
      )
