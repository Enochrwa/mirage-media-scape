...
export class LyricsService {
  ...
  public async getLyrics(track: Track) {
    ...
    const cached = this.db.prepare('SELECT * FROM lyrics_cache WHERE track_id = ?').get(track.id) as any;
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `).run(lyrics.track_id, lyrics.synced_lyrics, lyrics.plain_lyrics, lyrics.source, lyrics.fetched_at);
